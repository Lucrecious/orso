...
ERROR_XMACRO(ERROR_ANALYSIS_CANNOT_TAKE_ADDRESS_OF_CONSTANT, "cannot take address of constant", ANALYSIS)
ERROR_XMACRO(ERROR_ANALYSIS_INVALID_OPERAND_FOR_ADDRESS_OPERATOR, "invalid operand for address operator", ANALYSIS)
...
ERROR_XMACRO(ERROR_ANALYSIS_LHS_REQUIRES_TYPE_KNOWN_AT_COMPILE_TIME, "left hand side must be a type known at compile time", ANALYSIS)
ERROR_XMACRO(ERROR_ANALYSIS_INVALID_CAST, "invalid cast operand; cannot perform cast", ANALYSIS)
...
ERROR_XMACRO(ERROR_ANALYSIS_INVALID_DEREF_OPERAND, "only pointers can be dereferenced", ANALYSIS)
ERROR_XMACRO(ERROR_ANALYSIS_CANNOT_DEREFERENCE_VOIDPTR, "cannot deference a void pointer", ANALYSIS)
...
ERROR_XMACRO(ERROR_ANALYSIS_DEFINITION_DOES_NOT_EXIST, "definition does not exist", ANALYSIS)
ERROR_XMACRO(ERROR_ANALYSIS_DEFINITION_DOES_NOT_EXIST_IN_THE_SAME_RUN_SCOPE, "definition does not exist in the same run scope as where it's being evaluated", ANALYSIS)
ERROR_XMACRO(ERROR_ANALYSIS_CAN_ONLY_ACCESS_CONSTANTS_AND_GLOBALS, "only function locals, constants in higher scopes, and globals are accessible from within a function definition", ANALYSIS)
ERROR_XMACRO(ERROR_ANALYSIS_FUNCTION_PART_OF_CYCLICAL_DEPENDENCY, "function is defined circularly", ANALYSIS)
...
ERROR_XMACRO(ERROR_ANALYSIS_BLOCKS_TYPE_MISMATCH, "block types do not match", ANALYSIS)
