...
void orso_vm_call(OrsoVM* vm, OrsoFunction* function) {
    ...
    for (i32 i = 0; i < function->signature->type.function.argument_count; i++) {
        argument_slots += orso_type_slot_count(function->signature->type.function.argument_types[i]);
    }
...
static void call_object(OrsoVM* vm, OrsoObject* callee, i32 argument_slots) {
    if (callee->type->kind == ORSO_TYPE_FUNCTION) {
        ...
        i32 return_slot_size = orso_type_slot_count(function_obj->signature->type.function.return_type);
