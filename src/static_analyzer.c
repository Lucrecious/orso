...
static void scope_init(OrsoScope* scope, OrsoScopeType type, OrsoScope* outer, OrsoASTNode* creator_expression) {
    scope->outer = outer;
...
static OrsoScope* scope_copy_new(OrsoScope* scope) {
    ...
    scope_init(scope_copy, scope->type, NULL, scope->creator);
...
static void scope_merge(type_set_t* set, OrsoScope* scope, OrsoScope* a, OrsoScope* b) {
    if (!scope) {
...
static Entity* add_builtin_entity(OrsoAST* ast, OrsoSymbol* identifier, type_t* type, i32 value_index) {
    Entity* entity = ORSO_ALLOCATE(Entity);
    ...
    orso_symbol_table_set(&ast->builtins, identifier, ORSO_SLOT_P(entity));
...
static type_t* resolve_unary_type(OrsoAST* ast, TokenType operator, type_t* operand) {
...
static OrsoASTNode* implicit_cast(OrsoAST* ast, OrsoASTNode* operand, type_t* value_type) {
    ...
    implicit_cast->value_type_narrowed = value_type;
...
static type_t* get_folded_type(OrsoAST* ast, i32 index) {
    if (index < 0) {
        return &OrsoTypeInvalid;
    }
...
static i32 add_value_to_ast_constant_stack(OrsoAST* ast, slot_t* value, type_t* type) {
...
static bool can_call(type_t* type, OrsoASTNode** arguments) {
...
static bool is_block_compile_time_foldable(OrsoASTNode** block) {
    for (i32 i = 0; i < sb_count(block); i++) {
        OrsoASTNode* declaration = block[i];
    ...
    */
    if (ORSO_TYPE_IS_UNION(expression->value_type)) {
        ...
        if (state.mode & MODE_CONSTANT_TIME) {
            ...
            expression->value_type_narrowed = narrowed_type;
        }
    ...
    switch (expression->node_type) {
        ...
        case ORSO_AST_NODE_TYPE_EXPRESSION_BINARY: {
            ...
            switch (expression->operator.type) {
                ...
                case TOKEN_BAR: {
                    if (left->value_type_narrowed == right->value_type_narrowed && left->value_type_narrowed == &OrsoTypeType) {
                ...
                case TOKEN_GREATER_EQUAL: {
                    orso_binary_comparison_casts(left->value_type_narrowed, right->value_type_narrowed, &cast_left, &cast_right);
                    expression->value_type = &OrsoTypeBool;
                    break;
                }
                case TOKEN_BANG_EQUAL:
                case TOKEN_EQUAL_EQUAL: {
                    orso_binary_equality_casts(left->value_type_narrowed, right->value_type_narrowed, &cast_left, &cast_right);
                    expression->value_type = &OrsoTypeBool;
                    break;
                }
            ...
            if (expression->value_type_narrowed == &OrsoTypeUnresolved) {
                expression->value_type_narrowed = expression->value_type;
            }
        ...
        case ORSO_AST_NODE_TYPE_EXPRESSION_ASSIGNMENT: {
            ...
            if (lvalue_node->node_type == ORSO_AST_NODE_TYPE_EXPRESSION_ENTITY) {
                ...
                type_t* right_side_narrowed_type = expression->data.binary.rhs->value_type_narrowed;
        ...
        case ORSO_AST_NODE_TYPE_EXPRESSION_BLOCK: {
            ...
            scope_init(&block_scope, SCOPE_TYPE_BLOCK, state.scope, expression);
            ...
            if (block_return_guarentee != ORSO_NO_RETURN_GUARENTEED) {
                ...
                expression->value_type_narrowed = &OrsoTypeUndefined;
            } else {
                ...
                if (last_expression_statement == NULL) {
                    ...
                    expression->value_type_narrowed = last_expression_statement->data.expression->value_type_narrowed;
                }
            }
        ...
        case ORSO_AST_NODE_TYPE_EXPRESSION_STATEMENT: {
            ...
            if (expression->return_guarentee != ORSO_NO_RETURN_GUARENTEED) {
                ...
                expression->value_type_narrowed = expression->data.statement->data.expression->value_type_narrowed;
            }
            break;
        }
        ...
        case ORSO_AST_NODE_TYPE_EXPRESSION_BRANCHING: {
            ...
            if (branch_return_guarentee == ORSO_NO_RETURN_GUARENTEED) {
                type_t* else_block_type = &OrsoTypeVoid;
                type_t* else_block_narrowed_type = &OrsoTypeVoid;
                if (expression->data.branch.else_expression) {
                    else_block_type = expression->data.branch.else_expression->value_type;
                    else_block_narrowed_type = expression->data.branch.else_expression->value_type_narrowed;
                }
...
#define INITIAL_EXPRESSION (entity_declaration->data.declaration.initial_value_expression)
    ...
    type_t* declaration_type = &OrsoTypeUnresolved;
    ...
    unless (ORSO_TYPE_IS_UNION(entity_declaration->value_type)) {
        if (entity_declaration->value_type == &OrsoTypeUnresolved) {
            ...
            type_t* expression_type = entity_declaration->data.declaration.is_mutable ?
...
static void resolve_function_expression(
    ...
    OrsoScope function_parameter_scope;
    scope_init(&function_parameter_scope, SCOPE_TYPE_FUNCTION_PARAMETERS, state.scope, function_definition_expression);
    ...
    scope_free(&function_parameter_scope);
...
* 
*/
static void resolve_struct_definition(OrsoStaticAnalyzer* analyzer, OrsoAST* ast, AnalysisState state, OrsoASTNode* struct_definition) {
    OrsoScope struct_scope;
    ...
    if (incomplete_index < 0) {
        ...
        u64 size_in_slots = orso_bytes_to_slots(complete_struct_type->data.struct_.total_bytes);
...
static void resolve_statement(
    ...
    switch (statement->node_type) {
        ...
        case ORSO_AST_NODE_TYPE_STATEMENT_RETURN: {
            ...
            type_t* function_return_type = function_scope ? function_type->data.function.return_type : &OrsoTypeVoid;
...
bool orso_resolve_ast(OrsoStaticAnalyzer* analyzer, OrsoAST* ast) {
    ...
    scope_init(&global_scope, SCOPE_TYPE_MODULE, NULL, NULL);
